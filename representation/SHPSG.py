...
def SHPSG(Ar, D2_8, D9_15):
    """
    Params: a, b, c
        Let b = c = 1, then a = Ar * b = Ar
        So, the initial spheroid main axes are (a, b, c) = (Ar, 1, 1)
        scaler is (1.0 / Ar) ** (1/3)
        a_norm = a * scaler
        b_norm = b * scaler
    """
    ...
    fvec_sphere = -np.sqrt(np.pi / 6) * np.array(
        [[0, 0, 0], [-1, 1j, 0], [0, 0, np.sqrt(2)], [1, 1j, 0]]
    )
    Fvec = np.zeros((4, 3), dtype=complex)
    Fvec[:, 0] = a_norm * fvec_sphere[:, 0]
    Fvec[:, 1] = b_norm * fvec_sphere[:, 1]
    Fvec[:, 2] = b_norm * fvec_sphere[:, 2]
    ...
    for n in range(1, 16):
        ...
        N[n**2 : (n + 1) ** 2, :] = np.append(
            M1, np.flipud((-1) ** (n + 1) * M), axis=0
        )
    ...
    for n in range(16):
        R[n, :] = np.sqrt(
            sum(P[n**2 : (n + 1) ** 2, :] * Q[n**2 : (n + 1) ** 2, :]).real
        )
